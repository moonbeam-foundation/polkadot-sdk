...
use clap::Parser;
use sc_client_api::{Backend, HeaderBackend, TrieCacheContext};
use sc_executor::{
...
use sp_state_machine::backend::{BackendRuntimeCode, TryPendingCode};
...
impl PrecompileWasmCmd {
		...
		BA: Backend<B>,
	{
		...
		if backend.have_state_at(blockchain_info.finalized_hash, blockchain_info.finalized_number) {
			let state = backend.state_at(
				backend.blockchain().info().finalized_hash,
				TrieCacheContext::Untrusted,
			)?;

			precompile_and_serialize_versioned_wasm_runtime(
				HeapAllocStrategy::Static { extra_pages: heap_pages },
				// stable2603 added `TryPendingCode` (paritytech/polkadot-sdk#9947 vicinity).
				// Precompiling a runtime is a read-only operation, so use the
				// `No` variant — same context as runtime-API calls.
				&BackendRuntimeCode::new(&state, TryPendingCode::No).runtime_code()?,
